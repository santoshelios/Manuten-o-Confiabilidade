...
if st.session_state.logado:
    ...
    status_user = st.sidebar.multiselect(
        "Status Usuário",
        sorted(df.STATUS_USUARIO.dropna().unique())
    )

    efet = st.sidebar.multiselect(
        "Manutenção Efetuada",
        sorted(df.EFETUADA_MANUTENCAO.dropna().unique())
    )
    ...
    stat_pred = st.sidebar.multiselect(
        "Status Preditiva",
...
exec_real = round(
    executadas/(executadas+pendentes+nao_conf)*100,
    1
) if (executadas+pendentes+nao_conf)>0 else 0
...
with b2:
    ...
    top_n = st.slider(
        "Quantidade de defeitos no ranking",
        5,
        20,
        10
    )
...
tabela["STATUS"] = tabela["STATUS_PREDITIVA"].apply(
    lambda x:
...
tabela["PRIORIDADE"] = tabela["CRITICIDADE"].apply(
    lambda x:
    "🔴 Crítico" if x=="A"
